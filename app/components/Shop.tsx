...
const Shop = () => {
  return (
    <div>
      ...
      <div className="relative mt-8 sm:px-2 grid lg:grid-cols-3 gap-6 mb-20 md:grid-cols-2 sm:grid-cols-1">
        {items.map((item) => (
          <Link
            key={item.name}
            href={item.href}
           ...
           transition-transform duration-300 hover:scale-[1.02] cursor-pointer group block"
            style={{
              backgroundImage: `url(${item.url})`,
            }}
          >
            <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-black/80 to-transparent rounded-lg group-hover:from-black/90 transition-colors"></div>
            <div className="absolute left-6 top-5/6 -translate-y-1/2 flex flex-col gap-3">
              ...
              <p className="flex text-white text-lg font-bold gap-3 items-center group-hover:gap-4 transition-all">
